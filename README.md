...
# Instructions
Launch the VM: **everything will happen inside it.**

Connect on 42intra and start the evaluation from the VM.

Clone the repository and open it in VScode (still inside the VM).
...
I included the secrets/ directory AND the .env file on the .gitignore to prevent them from ever being pushed online. In the DEV_DOC, I gave an example of .env to copy into the right repository in order to facilitate both the development and the evaluation. I chose to use the secrets despite them not being necessary just for the love of the game. I thought having the SSL certificate and private key as secrets made sense.
