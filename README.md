...
I included the secrets/ directory AND the .env file on the .gitignore to prevent them from ever being pushed online. I prepared a local .env on my VM (itself on an external disk I always had with me) to copy into the right repository in order to facilitate both the development and the evaluation. I chose to use the secrets despite them not being necessary just for the love of the game.
